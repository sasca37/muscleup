...
1. 닭가슴살/상점/인벤토리를 MongoDB 백엔드 API로 이전할지 결정한다.
2. 펫/옷/악세사리 상품 목록의 실제 가격과 잠금 조건을 조정한다.
3. 장착한 옷/악세사리를 실제 캐릭터 이미지 위에 반영할 방식(별도 레이어 PNG, 캐릭터 합성 이미지, 단순 소품 표시)을 결정한다.
4. 백엔드 배포 대상(Render/Railway/Fly.io 등)을 확정한다.
5. 배포 환경에 `MONGODB_URI`, `KIS_APP_KEY`, `KIS_APP_SECRET`을 설정한다.
6. 프론트 배포 환경의 `VITE_API_BASE_URL`을 실제 백엔드 URL로 연결한다.
7. 실제 배포 환경에서 KIS 현재가와 운동 기록 API를 점검한다.
